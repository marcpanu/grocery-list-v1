...
export default function RecipeSearchModal({ isOpen, onClose, onSelect }: RecipeSearchModalProps) {
    ...
    const fetchRecipes = async () => {
      try {
        ...
        const fullRecipes = await Promise.all(
          recipePreviews.map(async (preview) => {
            try {
              return await getRecipe(preview.id);
            } catch (error) {
              console.error(`Failed to fetch full recipe ${preview.id}:`, error);
              return null;
            }
          })
        );
        // Filter out any null values and set the recipes
        setRecipes(fullRecipes.filter((recipe): recipe is Recipe => recipe !== null));
      } catch (error) {
  ...
  const filteredRecipes = recipes.filter((recipe) => {
    ...
    return (
      ...
      (recipe.cuisine?.some(c => c.toLowerCase().includes(searchLower)) ?? false)
    );
  });
              ...
              >
                ...
                <div className="mt-2 flex gap-2">
                  ...
                  {recipe.cuisine?.map(c => (
                    <span key={c} className="text-xs bg-green-100 text-green-800 px-2 py-1 rounded">
                      {c}
                    </span>
                  ))}
                </div>
